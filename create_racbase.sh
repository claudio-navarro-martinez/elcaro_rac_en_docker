...
createcontainer(){
    docker run -c $CPU_SHARE -m $MEMORY_LIMIT $DOCKER_CAPS --privileged  -e "container=docker" -v /sys/fs/cgroup:/sys/fs/cgroup -d --name racbase$1 oraclelinux:$1 /sbin/init
...
setupssh(){
    	docker run -c $CPU_SHARE -m $MEMORY_LIMIT $DOCKER_CAPS -d --privileged  -e "container=docker" -v /sys/fs/cgroup:/sys/fs/cgroup --name racbase$1 s4ragent/rac-on-docker:$1 /sbin/init
	sleep 35
	docker exec -i racbase$1 /bin/bash -c 'cat >/root/create_racbase.sh' <./create_racbase.sh
...
createnode(){
	...
	docker run -c $CPU_SHARE -m $MEMORY_LIMIT $DOCKER_CAPS -d --privileged  -e "container=docker" -v /sys/fs/cgroup:/sys/fs/cgroup -h ${nodename}.${DOMAIN_NAME} --name ${nodename} --dns=127.0.0.1 -v /lib/modules:/lib/modules -v /docker/media:/media ractest:racbase$2 /sbin/init
...
MYORACLESUPPORT_USERNAME=claudio.oami@gmail.com	
MYORACLESUPPORT_PASSWORD=oracle123
